...
- `src/test.cyr` — top-level test entry referenced by `cyrius.cyml [build].test`. Add unit cases here or in `tests/darshini.tcyr`.
- `tests/darshini.tcyr` — primary test suite (`cyrius test` auto-discovers).
- `tests/darshini.bcyr` — benchmarks (`cyrius bench`).
- `tests/darshini.fcyr` — fuzz harness (`cyrius fuzz`).

## Adding a feature

1. Edit `src/main.cyr` (or add a new module and `include` it).
2. Add a test case to `tests/darshini.tcyr`.
3. Run `cyrius test`.
